...
//		3. 모두의 점수 평균
		...
		for (int i = 0; i < sList.size(); i++) {
			double avg = (sList.get(i).english + sList.get(i).math + sList.get(i).korean) / 3;
			System.out.println(sList.get(i).name + "의 평균 점수는 " + avg);
		}
		double wholeavg = 0;
		for (int i = 0; i < sList.size(); i++) {
			wholeavg += (sList.get(i).english + sList.get(i).math + sList.get(i).korean);
		}
		System.out.println("모두의 평균 점수는 " + wholeavg / 5 / 3);
	}
}
